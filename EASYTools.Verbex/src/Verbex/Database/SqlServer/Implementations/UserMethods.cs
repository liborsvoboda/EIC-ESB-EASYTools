namespace Verbex.Database.SqlServer.Implementations
{
    ...
    internal class UserMethods : IUserMethods
    {
        ...
        public UserMethods(SqlServerDatabaseDriver driver)
        {
...
VALUES (
    N'{Sanitizer.Sanitize(user.Identifier)}',
    N'{Sanitizer.Sanitize(user.TenantId)}',
    N'{Sanitizer.Sanitize(user.Email)}',
    N'{Sanitizer.Sanitize(user.PasswordSha256)}',
...
SELECT identifier, tenant_id, email, password_sha256, first_name, last_name, is_admin, active, created_utc, last_update_utc
FROM users
WHERE tenant_id = N'{Sanitizer.Sanitize(tenantId)}' AND email = N'{Sanitizer.Sanitize(email)}';";
...
UPDATE users SET
    ...
    password_sha256 = N'{Sanitizer.Sanitize(user.PasswordSha256)}',
...
WHERE tenant_id = N'{Sanitizer.Sanitize(user.TenantId)}' AND identifier = N'{Sanitizer.Sanitize(user.Identifier)}';";
        ...
        public async Task<bool> DeleteByIdentifierAsync(string tenantId, string identifier, CancellationToken token = default)
        {
            ...
            string countQuery = $"SELECT COUNT(*) FROM users WHERE tenant_id = N'{Sanitizer.Sanitize(tenantId)}' AND identifier = N'{Sanitizer.Sanitize(identifier)}';";
            ...
            string query = $"DELETE FROM users WHERE tenant_id = N'{Sanitizer.Sanitize(tenantId)}' AND identifier = N'{Sanitizer.Sanitize(identifier)}';";
            await _Driver.ExecuteQueryAsync(query, true, token).ConfigureAwait(false);
            return true;
        }
        ...
        public async Task<long> DeleteByTenantAsync(string tenantId, CancellationToken token = default)
        {
            ...
            string countQuery = $"SELECT COUNT(*) FROM users WHERE tenant_id = N'{Sanitizer.Sanitize(tenantId)}';";
            DataTable countResult = await _Driver.ExecuteQueryAsync(countQuery, false, token).ConfigureAwait(false);
            long count = countResult.Rows.Count > 0 ? Convert.ToInt64(countResult.Rows[0][0]) : 0;

            string query = $"DELETE FROM users WHERE tenant_id = N'{Sanitizer.Sanitize(tenantId)}';";
            await _Driver.ExecuteQueryAsync(query, true, token).ConfigureAwait(false);
            return count;
        }
        ...
        public async Task<bool> ExistsByIdentifierAsync(string tenantId, string identifier, CancellationToken token = default)
        {
            ...
            string query = $"SELECT TOP 1 1 FROM users WHERE tenant_id = N'{Sanitizer.Sanitize(tenantId)}' AND identifier = N'{Sanitizer.Sanitize(identifier)}';";
            DataTable result = await _Driver.ExecuteQueryAsync(query, false, token).ConfigureAwait(false);
        ...
        public async Task<bool> ExistsByEmailAsync(string tenantId, string email, CancellationToken token = default)
        {
            ...
            string query = $"SELECT TOP 1 1 FROM users WHERE tenant_id = N'{Sanitizer.Sanitize(tenantId)}' AND email = N'{Sanitizer.Sanitize(email)}';";
            DataTable result = await _Driver.ExecuteQueryAsync(query, false, token).ConfigureAwait(false);
            return result.Rows.Count > 0;
        }
