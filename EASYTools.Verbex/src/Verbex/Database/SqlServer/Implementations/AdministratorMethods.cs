namespace Verbex.Database.SqlServer.Implementations
{
    ...
    /// <summary>
    /// SQL Server implementation of administrator methods.
    /// </summary>
    internal class AdministratorMethods : IAdministratorMethods
    {
        ...
        public AdministratorMethods(SqlServerDatabaseDriver driver)
        {
...
VALUES (
    N'{Sanitizer.Sanitize(administrator.Identifier)}',
    N'{Sanitizer.Sanitize(administrator.Email)}',
    N'{Sanitizer.Sanitize(administrator.PasswordSha256)}',
...
FROM administrators
WHERE identifier = N'{Sanitizer.Sanitize(identifier)}';";
...
SELECT identifier, email, password_sha256, first_name, last_name, active, created_utc, last_update_utc
FROM administrators
WHERE email = N'{Sanitizer.Sanitize(email)}';";
...
UPDATE administrators SET
    email = N'{Sanitizer.Sanitize(administrator.Email)}',
    password_sha256 = N'{Sanitizer.Sanitize(administrator.PasswordSha256)}',
...
WHERE identifier = N'{Sanitizer.Sanitize(administrator.Identifier)}';";
        ...
        public async Task<bool> DeleteByIdentifierAsync(string identifier, CancellationToken token = default)
        {
            ...
            string countQuery = $"SELECT COUNT(*) FROM administrators WHERE identifier = N'{Sanitizer.Sanitize(identifier)}';";
            ...
            string query = $"DELETE FROM administrators WHERE identifier = N'{Sanitizer.Sanitize(identifier)}';";
            await _Driver.ExecuteQueryAsync(query, true, token).ConfigureAwait(false);
            return true;
        }
        ...
        public async Task<bool> ExistsByIdentifierAsync(string identifier, CancellationToken token = default)
        {
            ...
            string query = $"SELECT TOP 1 1 FROM administrators WHERE identifier = N'{Sanitizer.Sanitize(identifier)}';";
            DataTable result = await _Driver.ExecuteQueryAsync(query, false, token).ConfigureAwait(false);
        ...
        public async Task<bool> ExistsByEmailAsync(string email, CancellationToken token = default)
        {
            ...
            string query = $"SELECT TOP 1 1 FROM administrators WHERE email = N'{Sanitizer.Sanitize(email)}';";
            DataTable result = await _Driver.ExecuteQueryAsync(query, false, token).ConfigureAwait(false);
            return result.Rows.Count > 0;
        }
