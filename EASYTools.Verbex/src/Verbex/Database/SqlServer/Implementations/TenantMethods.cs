namespace Verbex.Database.SqlServer.Implementations
{
    ...
    internal class TenantMethods : ITenantMethods
    {
        ...
        public TenantMethods(SqlServerDatabaseDriver driver)
        {
...
VALUES (
    N'{Sanitizer.Sanitize(tenant.Identifier)}',
...
WHERE identifier = N'{Sanitizer.Sanitize(tenant.Identifier)}';";
        ...
        public async Task<bool> DeleteByIdentifierAsync(string identifier, CancellationToken token = default)
        {
            ...
            string countQuery = $"SELECT COUNT(*) FROM tenants WHERE identifier = N'{Sanitizer.Sanitize(identifier)}';";
            ...
            string query = $"DELETE FROM tenants WHERE identifier = N'{Sanitizer.Sanitize(identifier)}';";
            await _Driver.ExecuteQueryAsync(query, true, token).ConfigureAwait(false);
            return true;
        }
        ...
        public async Task<bool> ExistsByIdentifierAsync(string identifier, CancellationToken token = default)
        {
            ...
            string query = $@"
SELECT TOP 1 1 FROM tenants WHERE identifier = N'{Sanitizer.Sanitize(identifier)}';";
        ...
        public async Task<bool> ExistsByNameAsync(string name, CancellationToken token = default)
        {
            ...
            string query = $@"
SELECT TOP 1 1 FROM tenants WHERE name = N'{Sanitizer.Sanitize(name)}';";
