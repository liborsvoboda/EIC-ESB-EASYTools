namespace Verbex.Database.Sqlite.Implementations
{
    ...
    using Sanitizer = Verbex.Database.Sqlite.Sanitizer;

    /// <summary>
    /// SQLite implementation of term methods.
    /// </summary>
    internal class TermMethods : ITermMethods
    {
        private readonly SqliteDatabaseDriver _Driver;

        public TermMethods(SqliteDatabaseDriver driver)
        {
        ...
        public async Task<string> AddOrGetAsync(string tablePrefix, string id, string term, CancellationToken token = default)
        {
            ...
            string insertQuery = $@"
INSERT OR IGNORE INTO {prefix}_terms (id, term, document_frequency, total_frequency, last_update_utc, created_utc)
...
FROM {prefix}_terms ORDER BY document_frequency DESC LIMIT {limit};";
        ...
        public async Task<Dictionary<string, string>> AddOrGetBatchAsync(string tablePrefix, Dictionary<string, string> terms, CancellationToken token = default)
        {
            ...
            sb.Append($"INSERT OR IGNORE INTO {prefix}_terms (id, term, document_frequency, total_frequency, last_update_utc, created_utc) VALUES ");
