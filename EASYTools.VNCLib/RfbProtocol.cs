...
// Copyright (C) 2011 Dalbish Farrjones @ twitter (Modified VncSharp for WPF)
//
...
namespace RemoteDesktopVNCWpf
{
	...
	{
        /// <summary>
        /// Reaise when Connected From VNC Server.
        /// </summary>
        public event EventHandler RfbConnectedFromServer;
		...
		protected TcpClient tcpClient;		// Network object used to communicate with host
        protected TcpListener tcpListener;  // Network object used to wait for a connection from Server
        ...
        private readonly object lockObj = new object(); // Used to Synchronized method writing a data into writer
		...
		/// <param name="port">The Port number on which to connectFromClient.  Usually this will be 5900, except in the case that the VNC Host is running on a different Display, in which case the Display number should be added to 5900 to determine the correct port.</param>
		public void Connect(string host, int port)
		{
			if (host == null) throw new ArgumentNullException("host");

			// Try to connectFromClient, passing any exceptions up to the caller, and if successful, 
			// wrap a big endian Binary Reader and Binary Writer around the resulting stream.
			tcpClient = new TcpClient();
			tcpClient.NoDelay = true;  // turn-off Nagle's Algorithm for better interactive performance with host.
        ...
        public void Listen(string host, int port)
        {
            if (host == null)
            {
                throw new ArgumentException("No Host Name.");
            }

            try
            {
                tcpListener = new TcpListener(Dns.GetHostEntry(host).AddressList[0], port);

                // Start Listening Thread
                Thread listenThread = new Thread(new ThreadStart(ListenCore));
                listenThread.Start();
            }
            catch (Exception ex) 
            {
        ...
        private void ListenCore()
        {
            try
            {
                ...
                if (RfbConnectedFromServer != null)
                {
                    RfbConnectedFromServer(this, EventArgs.Empty);
                }
            }
            catch (Exception)
            {
                ...
                if (zrleReader != null) zrleReader.Close();
                if (tcpClient != null) tcpClient.Close();
                
                if (FailedToListen != null)
                {
                    FailedToListen(this, EventArgs.Empty);
                }
            }
            finally
            {
		...
		/// </summary>
		/// <param name="shared">True if the server should allow other clients to connectFromClient, otherwise False.</param>
